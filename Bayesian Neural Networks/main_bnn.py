...
class semiGWGrunner(object):
    ...
    def init_weights(self, X_train, y_train):
        w1 = 1.0 / np.sqrt(X_train.shape[1] + 1) * torch.randn(self.config.training.batch_size,
                                                               X_train.shape[1] * self.config.model.bnn_latent_dim).to(
            self.device)
        b1 = torch.zeros((self.config.training.batch_size, self.config.model.bnn_latent_dim)).to(self.device)
        w2 = 1.0 / np.sqrt(self.config.model.bnn_latent_dim + 1) * torch.randn(self.config.training.batch_size,
                                                                               self.config.model.bnn_latent_dim).to(
            self.device)
        b2 = torch.zeros((self.config.training.batch_size, 1)).to(self.device)
        loglambda = torch.ones((self.config.training.batch_size, 1)).to(self.device) * np.log(np.random.gamma(1, 0.1))
        loggamma = torch.ones((self.config.training.batch_size, 1)).to(self.device)
        for i in range(self.config.training.batch_size):
            with torch.no_grad():
                ridx = np.random.choice(range(X_train.shape[0]), np.min([X_train.shape[0], 1000]), replace=False)
                y_hat = self.model.predict_y(
                    torch.cat([w1[i][None, :], b1[i][None, :], w2[i][None, :], b2[i][None, :]], dim=1).to(self.device),
                    X_train[ridx, :], self.y_mean, self.y_std, scale=False)
                loggamma[i] = -torch.log(torch.mean((y_hat - y_train[ridx, :]) ** 2, 1))
        return torch.cat([w1, b1, w2, b2, loggamma, loglambda], dim=1).to(self.device)
    ...
    def train(self):
        ...
        semi_sigma = torch.tensor(0.01)

        for ep in range(self.config.training.n_epoch):
            N0 = X_train.shape[0]
            batch = [i % N0 for i in
                     range(ep * self.config.training.data_batch_size, (ep + 1) * self.config.training.data_batch_size)]
            x = X_train[batch]
            y = y_train[batch]
            score_target = self.model.score(self.particle_weight, x, y, self.scale_sto)
            ...
            for i in range(self.config.training.f_iter):
                ...
                dup_x=self.particle_weight.clone()
                dup_x.requires_grad_(True)
                perturbed_x = dup_x + torch.randn_like(dup_x).to(self.device)* semi_sigma
                perturbed_x.requires_grad_(True)
                X_diff = perturbed_x - dup_x

                f_value = f_net(perturbed_x)
                f_optim.zero_grad()

                loss = torch.mean(torch.norm((f_value + X_diff / (semi_sigma ** 2)), dim=1, keepdim=True) ** 2)
                ...
                #     self.p_norm.requires_grad_(True)
                #     p_norm_optim.zero_grad()
                #     loss_p = - torch.mean(torch.sum(
                #         (torch.abs(f_value.detach()) ** (self.p_norm - 1)) ** (self.p_norm / (self.p_norm - 1)),
                #         dim=-1) / self.p_norm)
                #     loss_p.backward()
                #     self.p_norm.grad.clamp_(min=-0.2, max=0.2)
                #     p_norm_optim.step()
            ...
            with torch.no_grad():
                gdgrad = -f_net(self.particle_weight+ torch.randn_like(self.particle_weight).to(self.device)* semi_sigma)+self.model.score(self.particle_weight+ torch.randn_like(self.particle_weight).to(self.device)* semi_sigma, x, y, self.scale_sto)
                if ep == 0:
                    historical_grad = historical_grad + gdgrad ** 2
                else:
                    historical_grad = auto_corr * historical_grad + (1 - auto_corr) * gdgrad ** 2
            ...
            if ep % 10 == 0 or ep == (self.config.training.n_epoch - 1):
                test_rmse, test_llk = self.model.rmse_llk(self.particle_weight+ torch.randn_like(self.particle_weight).to(self.device)* semi_sigma, X_test, y_test, self.y_mean, self.y_std,
                                                          max_param=50.0)
                rmse_list.append(test_rmse)
                ll_list.append(test_llk)
                # p_norm_list.append(p_norm_item)
                # logging.info(
                #     "iter:[{}]/[{}]  pnorm: {:.4f}, rmse: {:.4f}, llk: {:.4f}".format(ep, self.config.training.n_epoch,
                #                                                                       p_norm_item, test_rmse, test_llk))

                logging.info(
                    "iter:[{}]/[{}]  rmse: {:.4f}, llk: {:.4f}".format(ep, self.config.training.n_epoch, test_rmse, test_llk))
        ...
        # p_norm_list.append(p_norm_item)
        # logging.info(
        #     "iter:[{}]/[{}]  pnorm: {:.4f}, after model selection: rmse_list: {:.4f}, ll_list: {:.4f}".format(ep,
        #                                                                                                       self.config.training.n_epoch,
        #                                                                                                       p_norm_item,
        #                                                                                                       test_rmse,
        #                                                                                                       test_llk))
        logging.info(
            "iter:[{}]/[{}]  after model selection: rmse_list: {:.4f}, ll_list: {:.4f}".format(ep,
                                                                                                              self.config.training.n_epoch,
                                                                                                              test_rmse,
                                                                                                              test_llk))

        # torch.save(np.array(rmse_list), os.path.join(self.config.log, "rmse_list_semigwg_1.pt"))
        # torch.save(np.array(ll_list), os.path.join(self.config.log, "ll_list_semigwg_1.pt"))
        # # torch.save(np.array(p_norm_list), os.path.join(self.config.log, "p_norm_list_1.pt"))
        #
...
class adasemiGWGrunner(object):
    ...
    def train(self):
        ...
        semisigma_lr = torch.tensor(0.00002)
        semi_sigma = torch.tensor(0.01)
        ...
        for ep in range(self.config.training.n_epoch):
            ...
            for i in range(self.config.training.f_iter):
                ...
                dup_x=self.particle_weight.clone()
                dup_x.requires_grad_(True)
                noise=torch.randn_like(dup_x).to(self.device)
            ...
            with torch.no_grad():
                ...
                noise= torch.randn_like(self.particle_weight).to(self.device)

                perturbed_x=self.particle_weight + noise * semi_sigma

                S = self.model.score(perturbed_x, x, y, self.scale_sto) - f_net(perturbed_x)
                semi_sigma_grad = torch.mean((S * noise).data)
                semi_sigma = semi_sigma + semisigma_lr * semi_sigma_grad
                print(semi_sigma, 'semisig')
...
def parse_config(path):
    ...
    # parser.add_argument("--config", type=str, default = "bnn_particle_boston.yml", help="Path to the config file")
    # parser.add_argument("--config", type=str, default = "bnn_particle_diabetes.yml", help="Path to the config file")
    # parser.add_argument("--config", type=str, default="bnn_particle_concrete.yml", help="Path to the config file")
    # parser.add_argument("--config", type=str, default = "bnn_particle_wine.yml", help="Path to the config file")
    # parser.add_argument("--config", type=str, default = "bnn_particle_ccpp.yml", help="Path to the config file")
    # parser.add_argument("--config", type=str, default="bnn_particle_protein.yml", help="Path to the config file")
    parser.add_argument("--config", type=str, default=f"bnn_particle_{path}.yml", help="Path to the config file")
    ...
    # parser.add_argument('--permutation_path', type=str, default='permutation/boston_permutation.pt', help='Path of permutation.')
    # parser.add_argument('--permutation_path', type=str, default='permutation/diabetes_permutation.pt', help='Path of permutation.')
    # parser.add_argument('--permutation_path', type=str, default='permutation/concrete_permutation.pt', help='Path of permutation.')
    # parser.add_argument('--permutation_path', type=str, default='permutation/wine_permutation.pt', help='Path of permutation.')
    # parser.add_argument('--permutation_path', type=str, default='permutation/ccpp_permutation.pt', help='Path of permutation.')
    # parser.add_argument('--permutation_path', type=str, default='permutation/protein_permutation.pt', help='Path of permutation.')
        ...
        for method in ('semi','adasemi'):
            if method=='semi':
                ...
                for itt in range(iter_n):
                    ...
                    config.log = os.path.join(config.store_path, 'correct_new_semiGWG_{}_semisigma_ini_{}_adap_{}_exp_times_{}'.format(config.permutation_path.replace("/","_"), 0.01, False, config.exp_time))
                    # config.log = os.path.join(config.store_path, 'correct_adasemiGWG_{}_semisigma_ini_{}_adap_{}_exp_times_{}'.format(config.permutation_path.replace("/","_"), 0.01, 2e-5, config.exp_time))
                    ...
                    handler2 = logging.FileHandler(os.path.join(config.log, f'stdout_{iter_n}times_newcorrect1.txt'))
                    ...
                    runner = semiGWGrunner(config)
                    # runner = adasemiGWGrunner(config)
                    runner.train()
                ...
                for itt in range(iter_n):
                    ...
                    # config.log = os.path.join(config.store_path, 'AdaGWG_{}_p_ini_{}_adap_{}_exp_times_{}'.format(config.permutation_path.replace("/","_"), config.model.p_norm, config.optim.p_adaptive, config.exp_time))
                    # config.log = os.path.join(config.store_path, 'correct_semiGWG_{}_semisigma_ini_{}_adap_{}_exp_times_{}'.format(config.permutation_path.replace("/","_"), 0.2, False, config.exp_time))
                    config.log = os.path.join(config.store_path,
                                              'correct_new_adasemiGWG_{}_semisigma_ini_{}_adap_{}_exp_times_{}'.format(
                                                  config.permutation_path.replace("/", "_"), 0.01, 2e-5,
                                                  config.exp_time))
