...
for p in p_list:
    ...
    for i in range(Epoch):
        ...
        if (i ) % check_frq == 0:
            ...
            # np.savetxt(
            #     f"../mog_unequalvar_2d_results/2d_adasemigwg_{i}_samples_fortraj_exact_sameseed1244_biased_var_{initvar}_perturbed.txt",
            #     X_plot)

            # if i== 1600:
            #     np.savetxt(
            #         f"../mog_unequalvar_2d_results/2d_adasemigwg_{i}_samples.txt",
            #         X_plot)
            #
            # if i== 2000:
            #     np.savetxt(
