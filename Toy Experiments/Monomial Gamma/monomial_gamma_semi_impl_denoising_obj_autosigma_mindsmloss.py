...
class PFGfast:
  ...
  def kl_distance(self,X_ori):
        ...
        x_true = exp.sampler(n_sample=1000).detach().cpu().numpy()
...
for p in p_list:
    ...
    pfg = PFGfast(exp, net, optim1, optim2,optim3)
...
# np.savetxt(
#         f'../monomial_gamma_correct_results_target1000/mono_semiGWG(DSM)_0.1_sigmaada_mindsmloss_semisigma_lr_{semisigma_lr}_seed_{seed}.txt',
#         kl_list)
#
#
# np.savetxt(
#         f'../monomial_gamma_correct_results_target1000/mono_semiGWG(DSM)_0.1_sigmaada_mindsmloss_semisigma_lr_{semisigma_lr}_semisigmalist_seed_{seed}.txt',
#         sigma_list)
